                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="card-header">
                                    <a href="{{route('admin.inv.slider.add')}}" class="btn btn-success round glow">
                                                    ...
                                                    @foreach($slider as $s)
                                                        <tr>
                                                            ...
                                                            <td class="text-center">
                                                                <button type="button" class="btn btn-icon rounded-circle btn-success glow mr-1 mb-1"  title="View Image" onClick="showImg('{{$s->id}}', '{{$s->file_img}}')" ><i class="bx bx-camera"></i></button>
                                                            </td>
                                                            <td>
                                                                @if ($s->active == 1)
                                                                <fieldset>
                                                                    <div class="checkbox checkbox-primary">
                                                                        <input type="checkbox" id="colorCheckbox{{$s->id}}" checked onchange="submitActive('{{$s->id}}','{{$s->judul_id}}')">
                                                                        <label for="colorCheckbox{{$s->id}}">Aktif</label>
                                                                    </div>
                                                                </fieldset>
                                                                @else
                                                                <fieldset>
                                                                    <div class="checkbox checkbox-primary">
                                                                        <input type="checkbox" id="colorCheckbox{{$s->id}}" onchange="submitActive('{{$s->id}}','{{$s->judul_id}}')">
                                                                        <label for="colorCheckbox{{$s->id}}">Aktif</label>
                                                                    </div>
                                                                </fieldset>
                                                                @endif
                                                            ...
                                                            <td class="text-center">
                                                                <a href="{{route('admin.inv.slider.edit',$s->id)}}"><button type="button" class="btn btn-icon rounded-circle btn-primary glow mr-1 mb-1" title="Edit"><i class="bx bxs-edit-alt"></i></button></a>
                                                                <button type="button" class="btn btn-icon rounded-circle btn-danger glow mr-1 mb-1" title="Delete" onClick="popModalDelete('{{ $s->id}}','{{ $s->judul_id }}')"><i class="bx bxs-trash"></i></button>
                                                            </td>
                                                        </tr>
                                                        <p hidden>{{$i++}}</p>
                                                    @endforeach
...
@section('script')
    <script>
        ...
        function submitDelete(){
            ...
            $.post( "{{route('admin.inv.slider.delete')}}", { "_token": "{{ csrf_token() }}", "id": idTarget })
                .done(function( res ) {
                    if (res==1){
                        ...
                        setTimeout(function(){
                            ...
                            window.location.href="{{route('admin.inv.slider')}}";
                        }, 1000);
                    } else {
        ...
        function submitActive(id,nama) {
            $(".cover-spin").fadeIn(500)
            up_nama = nama
            $.post( "{{route('admin.inv.slider.active')}}", { "_token": "{{ csrf_token() }}", "id": id })
                .done(function( res ) {
                    if (res==1){
        ...
        function showImg(id, file){
            ...
            $('#imgContainer').attr('src', "{{asset('uploads/SliderInvestment')}}/"+file+"");
