<?php $__env->startSection('title', 'BOPLBF - Berinvestasi'); ?>
...
<?php $__env->startSection('content'); ?>
	  ...
	  <main>
		<section id="Invest-registration">
			<h1 id="Invest-registration-title"><?php echo e(__('investment.investing')); ?></h1>
				...
				<div class="card-body">
					<form id="contact_form" >
						<div class="mb-4">
							...
							<input id="name" name="name" type="text" class="form-control" placeholder="<?php echo e(__('investment.investmentFormName')); ?>" aria-describedby="form-contact-name">
						</div>
						<div class="mb-4">
							<label for="contact-email" class="form-label sr-only">Email</label>
							<input type="email" class="form-control" id="contact-email" placeholder="<?php echo e(__('investment.investmentFormEmail')); ?>" aria-describedby="form-contact-email" required>
							<div id="email" name="email" class="form-text text-muted">We'll never share your email with anyone else.</div>
						</div>
						<div class="mb-4">
							<label for="contact-name" class="form-label sr-only">Subject</label>
							<input id="subject" name="subject" type="text" class="form-control form-control-lg" id="contact-subject" placeholder="<?php echo e(__('investment.investmentFormDetail')); ?>" aria-describedby="form-contact-subject">
						</div>
						<div class="mb-4">
							<label for="contact-message" class="form-label sr-only">Name</label>
							<textarea name="contact-message" id="message" name="message" cols="30" rows="3" placeholder="<?php echo e(__('investment.investmentFormMessage')); ?>" class="form-control" required></textarea>
						</div>

						<button type="button" class="btn btn-block btn-lg btn-primary" onclick="confirm_contact()"><?php echo e(__('investment.investmentFormSubmit')); ?></button>
					</form>
				</div>
			</div>
		</section>
...
<script>
	...
	function confirm_contact(){
		if($("#contact_form").valid()){
			...
			$.ajax({
				type:"POST",
				url : "<?php echo e(route('investasi.berinvestasi.submit')); ?>",
				headers: {
