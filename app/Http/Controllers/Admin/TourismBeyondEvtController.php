...
class TourismBeyondEvtController extends Controller
{

    public function TourismBLBEvent()
    {
        $data = TourismEvent::where('kategori','=','beyond')->get();
        return view('adminpage.tourism.blb_event',['data' => $data,'status'=> 0,'status_message' => '']);
    }

    public function TourismBLBEventAdd()
    {
        ...
        return view('adminpage.tourism.blb_event_add',$data);
    }

    public function TourismBLBEventStore(Request $request)
    {
        try{
            ...
            $data2 = TourismEvent::where('kategori','=','beyond')->get();
            if ($data){
                if ($request->hasfile('img')){
                    if(!File::exists(public_path().'/uploads/TourismEvent/BLB/'.$data->id.'/')){
                        File::makeDirectory(public_path().'/uploads/TourismEvent/BLB/'.$data->id.'/');
                    }
                    ...
                    $path = public_path()."/uploads/TourismEvent/BLB/".$data->id."/".$image_name;
                ...
                if(!empty($request['repeater'])){
                    foreach ($request['repeater'] as $key => $v){
                        if ($request->hasFile('repeater.'.$key.'.gallery')){
                            if(!File::exists(public_path().'/uploads/TourismEvent/BLB/'.$data->id.'/gallery/')){
                                File::makeDirectory(public_path().'/uploads/TourismEvent/BLB/'.$data->id.'/gallery/');
                            }
                            $file = $request->file('repeater.'.$key.'.gallery');
                            $name = 'gallery_'.Carbon::now()->format('YmdHis').$key.'.'.$file->getClientOriginalExtension();
                            $file->move(public_path().'/uploads/TourismEvent/BLB/'.$data->id.'/gallery/', $name);
                            $gallery = $name;
                ...
                return view('adminpage.tourism.blb_event',['data' => $data2,'status'=> 1,'status_message' => 'Sukses menambah Tour Package '.$request->judul_id]);
            }else{
                return view('adminpage.tourism.blb_event',['data' => $data2,'status'=> -1,'status_message' => 'Gagal menambah Tour Package '.$request->judul_id]);
            }
        } catch (\Exception $e) {
            $data2 = TourismEvent::where('kategori','=','beyond')->get();
            return view('adminpage.tourism.blb_event',['data' => $data2,'status'=> -1,'status_message' => 'Gagal menambah Tour Package : Silahkan gunakan judul yang lain']);
        }
    }


    public function TourismBLBEventDelete(Request $request)
    {
    ...
    public function TourismBLBEventEdit($id)
    {
        ...
        return view('adminpage.tourism.blb_event_edit',$data);
    }

    public function TourismBLBEventUpdate(Request $request)
    {
        try{
            $slug = str_slug($request->judul_id);
            // $date = Carbon::createFromFormat("m-d-Y H:i", $request->waktu_event);
            ...
            if ($data){
                if ($request->hasfile('img')){
                    if(!File::exists(public_path().'/uploads/TourismEvent/BLB/'.$request->id.'/')){
                        File::makeDirectory(public_path().'/uploads/TourismEvent/BLB/'.$request->id.'/');
                    }
                    ...
                    $image = base64_decode($image);
                    $image_name= 'thumb_'.Carbon::now()->format('YmdHis').'.jpg';
                    $path = public_path()."/uploads/TourismEvent/BLB/".$request->id."/".$image_name;
                    file_put_contents($path, $image);
                ...
                if(!empty($request['repeater'])){
                    foreach ($request['repeater'] as $key => $v){
                        if ($request->hasFile('repeater.'.$key.'.gallery')){
                            if(!File::exists(public_path().'/uploads/TourismEvent/BLB/'.$request->id.'/gallery/')){
                                File::makeDirectory(public_path().'/uploads/TourismEvent/BLB/'.$request->id.'/gallery/');
                            }
                            $file = $request->file('repeater.'.$key.'.gallery');
                            $name = 'gallery_'.Carbon::now()->format('YmdHis').$key.'.'.$file->getClientOriginalExtension();
                            $file->move(public_path().'/uploads/TourismEvent/BLB/'.$request->id.'/gallery/', $name);
                            $gallery = $name;
                            TourismRelasiImg::create([
                ...
                return view('adminpage.tourism.blb_event',['data' => $data2,'status'=> 1,'status_message' => 'Sukses edit '.$request->judul_id]);
            }else{
                return view('adminpage.tourism.blb_event',['data' => $data2,'status'=> -1,'status_message' => 'Gagal edit '.$request->judul_id]);
            }
        } catch (\Exception $e) {
            $data2 = TourismEvent::where('kategori','=','beyond')->get();
            return view('adminpage.tourism.blb_event',['data' => $data2,'status'=> -1,'status_message' => 'Gagal edit Tour Package : Silahkan gunakan judul yang lain']);
        }
    }
