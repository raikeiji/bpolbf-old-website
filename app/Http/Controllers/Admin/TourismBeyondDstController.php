...
class TourismBeyondDstController extends Controller
{

    public function TourismBLBDestinasi()
    {
        $data = TourismDestination::where('kategori','=','beyond')->get();
        return view('adminpage.tourism.blb_destinasi',['data' => $data,'status'=> 0,'status_message' => '']);
    }

    public function TourismBLBDestinasiAdd()
    {
        ...
        return view('adminpage.tourism.blb_destinasi_add',$data);
    }

    public function TourismBLBDestinasiStore(Request $request)
    {
        try{
            //buat slug
            ...
            $data2 = TourismDestination::where('kategori','=','beyond')->get();
            if ($data){
                if ($request->hasfile('img')){
                    if(!File::exists(public_path().'/uploads/TourismDestinasi/BLB/'.$data->id.'/')){
                        File::makeDirectory(public_path().'/uploads/TourismDestinasi/BLB/'.$data->id.'/');
                    }
                    ...
                    $path = public_path()."/uploads/TourismDestinasi/BLB/".$data->id."/".$image_name;
                ...
                if(!empty($request['repeater'])){
                    foreach ($request['repeater'] as $key => $v){
                        if ($request->hasFile('repeater.'.$key.'.gallery')){
                            if(!File::exists(public_path().'/uploads/TourismDestinasi/BLB/'.$data->id.'/gallery/')){
                                File::makeDirectory(public_path().'/uploads/TourismDestinasi/BLB/'.$data->id.'/gallery/');
                            }
                            $file = $request->file('repeater.'.$key.'.gallery');
                            $name = 'gallery_'.Carbon::now()->format('YmdHis').$key.'.'.$file->getClientOriginalExtension();
                            $file->move(public_path().'/uploads/TourismDestinasi/BLB/'.$data->id.'/gallery/', $name);
                            $gallery = $name;
                ...
                return redirect()->route('admin.t.blb.destinasi',['data' => $data2,'status'=> 1,'status_message' => 'Sukses menambah data '.$request->judul_id]);
            }else{
                return redirect()->route('admin.t.blb.destinasi',['data' => $data2,'status'=> -1,'status_message' => 'Gagal menambah data '.$request->judul_id]);
            }
        } catch (\Exception $e) {
            $data2 = TourismDestination::where('kategori','=','beyond')->get();
            return view('adminpage.tourism.blb_destinasi',['data' => $data2,'status'=> -1,'status_message' => 'Gagal menambahkan data : Silahkan gunakan judul lain']);
        }
    }

    public function TourismBLBDestinasiDelete(Request $request)
    {
    ...
    public function TourismBLBDestinasiHomepage(Request $request)
    {
        $data = TourismDestination::where('id','=',$request->id)
            ->first();
    ...
    public function TourismBLBDestinasiEdit($id)
    {
        ...
        return view('adminpage.tourism.blb_destinasi_edit',$data);
    }

    public function TourismBLBDestinasiUpdate(Request $request)
    {
        try{
            //buat slug
            $slug = str_slug($request->judul_id,'-');
            ...
            if ($data){
                if ($request->hasfile('img')){
                    if(!File::exists(public_path().'/uploads/TourismDestinasi/BLB/'.$request->id.'/')){
                        File::makeDirectory(public_path().'/uploads/TourismDestinasi/BLB/'.$request->id.'/');
                    }
                    ...
                    $image = base64_decode($image);
                    $image_name= 'thumb_'.Carbon::now()->format('YmdHis').'.png';
                    $path = public_path()."/uploads/TourismDestinasi/BLB/".$request->id."/".$image_name;
                    file_put_contents($path, $image);
                ...
                if(!empty($request['repeater'])){
                    foreach ($request['repeater'] as $key => $v){
                        if ($request->hasFile('repeater.'.$key.'.gallery')){
                            if(!Storage::exists(public_path().'/uploads/TourismDestinasi/BLB/'.$request->id.'/gallery/')){
                                Storage::makeDirectory(public_path().'/uploads/TourismDestinasi/BLB/'.$request->id.'/gallery/');
                            }
                            $file = $request->file('repeater.'.$key.'.gallery');
                            $name = 'gallery_'.Carbon::now()->format('YmdHis').$key.'.'.$file->getClientOriginalExtension();
                            $file->move(public_path().'/uploads/TourismDestinasi/BLB/'.$request->id.'/gallery/', $name);
                            $gallery = $name;
                            TourismRelasiImg::create([
                ...
                return view('adminpage.tourism.blb_destinasi',['data' => $data2,'status'=> 1,'status_message' => 'Sukses mengedit data '.$request->judul_id]);
            }else{
                return view('adminpage.tourism.blb_destinasi',['data' => $data2,'status'=> -1,'status_message' => 'Gagal mengedit data '.$request->judul_id]);
            }
        } catch (\Exception $e) {
            $data2 = TourismDestination::where('kategori','=','beyond')->get();
            return view('adminpage.tourism.blb_destinasi',['data' => $data2,'status'=> -1,'status_message' => 'Gagal mengedit data : Silahkan gunakan judul lain']);
        }
    }
